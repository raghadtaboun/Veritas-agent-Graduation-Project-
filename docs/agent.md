...
The backend, agents, MCP server, and tests are all written in Python. No other backend language is permitted. The frontend is Streamlit Python only.
...
**Environment bootstrap is mandatory.** Every process entry point (`mcp_server/server.py`, `api/main.py`, `agents/graph.py`, `tests/conftest.py`) must import `config/env_bootstrap.py` at the very top, before any other project import and before any LLM SDK import:
