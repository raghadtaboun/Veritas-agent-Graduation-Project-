...
The system serves three fixed news sections: **Middle East**, **Libya**, and **World**, each with its own data sources, query configuration, and recommended refresh cadence. Each section has an independent `update_every_hours` value defined in `config/sections.py` — Middle East and World every 6 hours, Libya every 4 hours. This value is informational: the pipeline is triggered manually via `run_pipeline(section)`; it does not run on an automated schedule.

---

## 2. Core Architectural Layers

The system is composed of four distinct layers. Each layer has a single, well-defined responsibility, and communication between layers follows strict boundaries.

### Layer 1 — Orchestration Layer (LangGraph Pipeline)
...
### Layer 2 — Agent Layer (The Reasoning Plane)
...
### Layer 3 — MCP Tool Layer (The Data Plane)
...
```
[Manual Pipeline Trigger] — run_pipeline(section) invoked manually per section
     │
     ▼
...
- `gemini_generate_with_fallback(prompt, task_type, ...)` — text generation through a per-task fallback chain, served via the Gemini / Google GenAI API.
- `gemini_embed(text)` — 768-dim embedding via `models/gemini-embedding-001`.
- `groq_generate(prompt, model, ...)` — fast relevance and classification tasks via Groq/Llama.

### Fallback chains per task

Each task type maps to an ordered fallback chain. On a 429 / quota-exhausted error the client first rotates across the eight-key pool for the **same** model, then advances to the next model in the chain. Every task chain currently resolves to a single model, `gemma-4-31b-it`; the multi-key rotation is what protects the pipeline from a single-key outage.
...
A single bootstrap module `config/env_bootstrap.py` is imported at every process entry point (MCP server, FastAPI, agent tests) before any SDK import. It pops conflicting shell-level keys (`GOOGLE_API_KEY`, `GOOGLE_GENAI_API_KEY`, `GOOGLE_APPLICATION_CREDENTIALS`), loads `.env` with `override=True`, validates required keys, and re-sets `GOOGLE_API_KEY = GEMINI_API_KEY` so every transitively-imported SDK uses the project's dedicated key.
...
The system's accuracy is measured against a manually annotated dataset of 60 Arabic articles (`evaluation/dataset.json`), distributed across all five bias labels (12 articles per label). The primary metrics are accuracy and macro-F1 (the unweighted mean of the five per-label F1 scores); a malformed model output is recorded as `parse_error` and counts as incorrect. The academic target is F1 ≥ 0.65, chosen to be honest about current LLM limitations on Arabic-language content rather than overclaiming performance.

The Bias Agent exposes a `classify_single(text)` method and a single-article prompt builder that `evaluation/evaluate_bias.py` reuses verbatim. The harness runs that production prompt across several backends — local Ollama Gemma sizes and a hosted `gemini-3.5-flash` reference — parsing each response and scoring it against the human labels without touching the database, minus the comparative cluster context production uses.

A second evaluation harness, `evaluation/evaluate_clustering.py`, tests the Clustering Agent's ability to group same-event articles despite differing bias labels. It runs the dataset articles through the production clustering logic and checks whether the known same-event groups defined in `evaluation/clustering_ground_truth.json` are placed into a single cluster. This validates that clustering relies on semantic similarity (entity overlap + embeddings) rather than surface title matching — the hardest cases being same-event pairs with lexically different titles across bias framings.
