...
Build the `get_embedding` tool using Gemini embeddings. (`text-embedding-004` was the originally-planned default, but it is unavailable on the project API key, so the model actually used is `models/gemini-embedding-001`.) It generates a 768-dimension float vector for the input text. Input text is truncated to 3,000 characters before embedding. The result is a dict with an `embedding` key containing the float list. Check Redis for a cached result before calling the API. Cache results with a 7-day TTL since embeddings are deterministic for the same input.
...
- `gemini_embed(text) -> dict` — single embedding call. (`text-embedding-004` was the planned default; the project `.env` sets `models/gemini-embedding-001`, the model actually used.) Returns `{"embedding": list[float], "dimension": 768}` on success.
...
Add `from config.env_bootstrap import bootstrap_env; bootstrap_env()` at the top of `mcp_server/server.py`, `api/main.py`, and `tests/conftest.py` (create `conftest.py` if it does not yet exist) **before** any `google.genai` or `groq` import.
...
3. `grep -l "load_dotenv()" mcp_server/ api/ agents/ 2>/dev/null` → zero matches. Every entry point uses `bootstrap_env()` instead.
...
**Goal:** The system is evaluated against the annotated dataset across both bias classification and clustering correctness, and the project is fully documented.
...
### Step 6.2 — Implement the bias evaluation script

Build the evaluation script. (Planned here as `evaluation/evaluate.py`; implemented as `evaluation/evaluate_bias.py` — see `summaries/phase_6/summary.md` Section C.) It must load the dataset, run the production bias prompt (reused verbatim from `agents/bias_agent.py`) over each article, compare predictions to human labels, and print a full classification report. (The plan originally specified a weighted F1-Score via `sklearn`; the implementation computes accuracy and macro-F1 with manual metrics — see Phase 6 Section C.) The script must produce results that are directly citable in the thesis. The target F1-Score for Arabic content is ≥ 0.65 — lower than English-language benchmarks due to the limited Arabic training data in the models used. This gap is itself an academically significant finding and should be documented as such.

The evaluation spans a **multi-size model comparison**: run the same 60-article dataset through multiple Gemma model sizes (`gemma4:e4b-mlx`, `gemma4:12b-mlx`, `gemma4:26b`, `gemma4:31b-mlx`, plus their QAT variants) and a hosted `gemini-3.5-flash` reference, and record accuracy / macro-F1 per model. This comparison is a secondary academic finding — it shows how model capacity affects Arabic bias-classification performance.
...
### Step 6.3 — Clustering Evaluation

Build `evaluation/evaluate_clustering.py`. This harness tests whether the Clustering Agent correctly groups same-event articles — articles covering the same real-world event but rewritten with different bias labels — into a single cluster.

Load the 60 dataset articles from `evaluation/dataset.json` and the known same-event groups from `evaluation/clustering_ground_truth.json`. Run all articles through the existing clustering logic (the same three-condition pipeline used in production: time window ≤ 72h, cosine similarity ≥ 0.82, entity overlap ≥ 2). For each same-event group defined in `clustering_ground_truth.json`, verify that all member articles were assigned to the same cluster.

The primary metric is the **number of same-event groups correctly clustered** out of the total defined groups. Give special attention to the **hard cases** — same-event article pairs where titles differ across bias framings — as these test whether the system relies on semantic similarity (entities + embeddings) rather than surface title matching. A group is correctly clustered only if all its members land in one cluster (not split across multiple clusters or left as singletons).

This step validates a distinct capability of the Clustering Agent that the bias F1 evaluation does not cover: that semantic grouping is robust to lexical variation introduced by different bias framings of the same event.
...
**Phase 6 Success Criteria:** `pytest tests/` passes fully. Bias F1-Score on the 60-article dataset is computed, documented, and reported across multiple model sizes. Clustering same-event grouping accuracy is computed and documented, with hard-case results noted separately. All phase `summary.md` files are present, including Phase 4.5.
...
| 6 | Evaluation (bias F1 + clustering) + Docs | F1-Score computed across model sizes, clustering same-event accuracy documented |
