...
- EMPHASIS: What does each article foreground in its headline and lead?
  (ما الذي يُبرزه كل مقال في عنوانه ومقدّمته؟)
- OMISSION: What facts in OTHER sources are absent or buried in each?
  (ما الحقائق الموجودة في مصادر أخرى والغائبة أو المدفونة في كل مقال؟)
- VOCABULARY: What loaded Arabic terms does each use? Common contrasts:
  (ما المصطلحات العربية المشحونة التي يستخدمها كل مقال؟)
...
- SOURCING: Whom does each quote (officials, opposition, foreign govs, NGOs, eyewitnesses)?
  (مَن يقتبس كل مقال؟ مسؤولون، معارضة، حكومات أجنبية، منظمات، شهود عيان؟)
- NARRATIVE: Who is implicitly cast as legitimate vs illegitimate?
  (مَن يُصوَّر ضمنياً كشرعيّ مقابل غير شرعيّ؟)
...
R1. If charged language appears ONLY inside quotes and sides are presented
    ...
    ▸ إذا ظهرت اللغة المشحونة داخل الاقتباسات فقط وعُرضت الأطراف بتناسب ← neutral.
...
R2. If LOADED VOCABULARY is 'none' BUT sourcing is one-sided or counter-views
    ...
    ▸ إذا غابت المفردات المشحونة لكن المصادر أحادية أو الرأي المقابل غائب ← اختر التصنيف المائل.
...
R3. If the tone is calm/analytical BUT actors are legitimized/delegitimized
    ...
    ▸ إذا كانت النبرة هادئة لكن الأطراف تُشرَّع/تُجرَّم بشكل غير متكافئ ← اختر التصنيف المائل.
...
R4. If the article criticizes EXTERNAL/Western interference in defense of an
    ...
    ▸ إذا انتقد المقال تدخّلاً خارجياً/غربياً دفاعاً عن مصلحة عربية ← pan_arab.
      إذا انتقد حكومةً داخلية محدّدة ← opposition.
...
R5. A Western-sourced piece that criticizes Western actors is critical/opposition
    ...
    ▸ مصدرٌ غربيّ ينتقد الغرب = نقديّ/معارض أو محايد — وليس western_aligned بسبب أصله فقط.
...
▸ confidence (your certainty about the label, 0.0 to 1.0):
    ...
    NOTE: a well-supported neutral (balanced sourcing, charged words only in quotes)
    deserves HIGH confidence — do not under-rate clear neutrality.
